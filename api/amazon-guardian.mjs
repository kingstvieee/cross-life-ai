...
import { runGuardianOnBedrock } from '../competition/Amazon2026/src/aws-strands-adapter.mjs';
...
export default async function handler(request, response) {
  ...
  const input = String(body.input || 'Guardian, get me ready for tomorrow').slice(0, 2000);

  // This mode is intentionally separate from the deterministic judge path.
  // A successful response here is evidence that Strands actually invoked Amazon Bedrock.
  if (body.mode === 'bedrock') {
    try {
      const result = await runGuardianOnBedrock(input);
      return response.status(200).json({
        ok: true,
        mode: 'bedrock',
      ...
      console.error('Guardian Bedrock invocation failed', error);
      return response.status(502).json({
        ...
        provider: 'Amazon Bedrock',
        framework: 'Strands Agents SDK',
        error: error instanceof Error ? error.message : 'Bedrock invocation failed'
      });
    }
  }
  ...
  const raw = mode === 'judge'
    ? judgeScenario()
    : buildGuardianPlan(input, { source: 'Alexa+ simulated experience' });
