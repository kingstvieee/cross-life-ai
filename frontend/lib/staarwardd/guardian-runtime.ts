...
export class GuardianRuntime {
  ...
  private observation(event: GuardianEvent) { if (event.type === "PORTAL_OPENED") return `The ${String(event.payload.portal ?? "current")} world is active. I am keeping continuity in the background.`; if (event.type === "USER_REQUESTED") return "I have the request and I am checking the context before suggesting a next move."; return `I noticed ${event.type.toLowerCase().replaceAll("_", " ")}.`; }
  private recommendation(event: GuardianEvent) { if (event.type === "USER_REQUESTED") return "One useful move is ready for your review."; return "I will hold this quietly unless it becomes more relevant."; }
}
