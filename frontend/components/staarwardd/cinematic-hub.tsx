...
export function CinematicHub() {
  ...
  return (
    <View style={styles.root}>
      ...
      <SafeAreaView style={styles.safe} edges={["top", "left", "right"]}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.commandField}>
            ...
            <GuardianCharacter state={fieldAwake ? "portalSelection" : "idle"} mood={fieldAwake ? "excited" : "focused"} portalMode="hub" size={254} />
            {fieldAwake && (
            ...
            <View style={styles.gatewayField}>{PORTALS.map((portal, index) => <Gateway key={portal.id} portal={portal} index={index} awake={fieldAwake} onPress={() => enter(portal.id)} />)}</View>
            <View style={styles.commandCopy}>
              <Text style={styles.commandKicker}>{fieldAwake ? "THE FIELD IS AWAKE" : "GUARDIAN AT CENTER"}</Text>
              <Text style={styles.commandPrompt}>{fieldAwake ? "Choose your first world." : "Let the Guardian awaken the gateways."}</Text>
          ...
          <Text style={styles.note}>STAARWARDD v1.3 · Native cinematic preview</Text>
        </ScrollView>
      </SafeAreaView>
      ...
      <InfoModal open={aboutOpen} onClose={() => setAboutOpen(false)} />
...
function Gateway({ portal, index, awake, onPress }: { portal: (typeof PORTALS)[number]; index: number; awake: boolean; onPress: () => void }) {
  ...
  return <Animated.View style={[styles.gatewayWrap, pos, { opacity: form, transform: [{ translateY: rise }, { scale }, { scale: chargeScale }] }]}><Pressable accessibilityRole="button" accessibilityLabel={`Enter ${portal.name}`} onPress={onPress} style={({ pressed }) => [styles.gateway, { borderColor: portal.color, shadowColor: portal.color }, awake && styles.gatewayAwake, pressed && styles.gatewayPressed]}><Image source={portal.image} resizeMode="cover" style={styles.gatewayImage} /><View style={[styles.gatewayTint, { backgroundColor: portal.color }]} /><Animated.View pointerEvents="none" style={[styles.gatewayCharge, { opacity: chargeOpacity, borderColor: portal.color }]} /><View style={styles.gatewayShade} /><Text style={[styles.gatewayGlyph, { color: portal.accent }]}>{portal.glyph}</Text><Text numberOfLines={1} style={styles.gatewayName}>{portal.name}</Text></Pressable></Animated.View>;
}

function gatewayPosition(index: number) { const angle = (index / 7) * Math.PI * 2 - Math.PI / 2; return { left: `${50 + Math.cos(angle) * 39}%` as `${number}%`, top: `${48 + Math.sin(angle) * 39}%` as `${number}%` }; }
function InfoModal({ open, onClose }: { open: boolean; onClose: () => void }) { return <Modal transparent visible={open} animationType="fade" onRequestClose={onClose}><View style={styles.back}><View style={styles.modal}><Text style={styles.modalKicker}>ABOUT THIS BUILD</Text><Text style={styles.modalTitle}>A cinematic local preview.</Text><Text style={styles.modalCopy}>Guardian motion, original audio, and device text-to-speech are optional. This build does not claim a connected account, live AI, hardware, event feed, wearable, home device, or external action.</Text><Pressable accessibilityRole="button" onPress={onClose} style={styles.modalButton}><Text style={styles.modalButtonText}>UNDERSTOOD</Text></Pressable></View></View></Modal>; }
