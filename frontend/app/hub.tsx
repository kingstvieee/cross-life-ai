    ...
    <SafeAreaView style={styles.root} edges={['top']} testID="hub-root">
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <Pressable style={styles.companionPill} onPress={cycleCompanion} testID="companion-switcher">
              <Ionicons name="sparkles-outline" size={13} color="#00E5FF" />
            ...
            <Pressable style={styles.iconBtn} onPress={() => router.push('/settings')} testID="settings-btn">
              <Ionicons name="settings-outline" size={18} color="#F3F4F6" />
            </Pressable>
          </View>
        </View>
        ...
        <View style={{ width: SW, height: RADIAL_H }}>
          {/* Ambient center glow + base lines */}
          <Canvas style={StyleSheet.absoluteFill}>
            <Circle cx={CX} cy={CY} r={RADIUS * 1.05} color="#00E5FF" opacity={0.06}>
              <BlurMask blur={70} style="normal" />
            </Circle>
            {positions.map((p) => (
              <Line key={p.id} p1={vec(CX, CY)} p2={vec(p.x, p.y)} color="rgba(255,255,255,0.07)" strokeWidth={1} />
            ))}
            <Circle cx={CX} cy={CY} r={RADIUS} color="rgba(255,255,255,0.06)" style="stroke" strokeWidth={1} />
          </Canvas>
          ...
          {activeLines.length > 0 && (
            <Animated.View style={[StyleSheet.absoluteFill, lineStyle]} pointerEvents="none">
              <Canvas style={StyleSheet.absoluteFill}>
                {activeLines.map((l, i) => (
                  <Line key={i} p1={vec(l.from.x, l.from.y)} p2={vec(l.to.x, l.to.y)} color={theme.color.energy} strokeWidth={2.5}>
          ...
          <Animated.View style={[styles.orbWrap, orbStyle, { left: CX - 70, top: CY - 70 }]}>
            <Canvas style={{ width: 140, height: 140 }}>
              <Circle cx={70} cy={70} r={56} color={theme.color.energy} opacity={0.5}>
                <BlurMask blur={34} style="normal" />
              </Circle>
              <Circle cx={70} cy={70} r={46} color={theme.color.emerald} opacity={0.22}>
                <BlurMask blur={20} style="normal" />
              </Circle>
              <Circle cx={70} cy={70} r={40} color="#EAFDFF" />
              <Circle cx={70} cy={70} r={40} color="rgba(0,229,255,0.6)" style="stroke" strokeWidth={1} />
              <Circle cx={70} cy={70} r={52} color="rgba(0,229,255,0.25)" style="stroke" strokeWidth={1} />
            </Canvas>
            <Pressable
            ...
            >
              <Ionicons name="flash" size={22} color="#0A0B0E" />
            ...
            return (
              <View key={p.id} style={[styles.nodeWrap, { left: p.x - NODE / 2, top: p.y - NODE / 2 }]}>
                <Pressable onPress={() => openPortal(p.id)} testID={`portal-node-${p.id}`}>
                  <BlurView intensity={30} tint="dark" style={[styles.node, isSource && styles.nodeActive]}>
                    <Ionicons name={PORTAL_ICONS[p.id]} size={20} color={meta.accent} />
                  </BlurView>
        ...
        {paused ? (
          ...
          <Pressable style={[styles.banner, styles.bannerActive]} onPress={() => openPortal(topSignal.sourcePortal)} testID="signal-banner">
...
const styles = StyleSheet.create({
  ...
  brand: { fontSize: 15, fontWeight: '800', letterSpacing: 4, color: '#FFFFFF' },
  sub: { fontSize: 10, letterSpacing: 2, color: '#00E5FF', marginTop: 3, fontWeight: '700' },
  companionPill: {
  ...
  companionText: { fontSize: 12, fontWeight: '700', color: '#F3F4F6', letterSpacing: 0.5 },
  iconBtn: {
  ...
  orbLabel: { fontSize: 8, letterSpacing: 2, fontWeight: '800', color: '#0A0B0E', marginTop: 3 },
  ...
  node: {
    ...
    borderWidth: 1, borderColor: 'rgba(255,255,255,0.15)', backgroundColor: 'rgba(21,23,29,0.72)',
  },
  ...
  nodeDot: {
    ...
    backgroundColor: theme.color.energy, borderWidth: 1.5, borderColor: '#0A0B0E',
  },
  nodeLabel: { marginTop: 6, fontSize: 10, fontWeight: '700', letterSpacing: 0.5, color: '#D1D5DB' },
  banner: {
    marginHorizontal: 24, marginTop: 8, padding: 18, borderRadius: 20,
    backgroundColor: '#15171D', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)',
  },
  bannerActive: { borderColor: 'rgba(0,229,255,0.35)' },
  liveDot: { width: 8, height: 8, borderRadius: 999, backgroundColor: theme.color.energy },
  bannerKicker: { fontSize: 10, letterSpacing: 2.5, fontWeight: '800', color: '#9CA3AF' },
  bannerText: { marginTop: 8, fontSize: 15, lineHeight: 21, color: '#F3F4F6', fontWeight: '600' },
  bannerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 },
  bannerMeta: { fontSize: 11, color: '#9CA3AF', fontWeight: '600', flex: 1 },
  bannerCta: { fontSize: 12, fontWeight: '800', letterSpacing: 1.5, color: '#00E5FF' },
  actions: { paddingHorizontal: 24, marginTop: 16, gap: 10 },
  primaryBtn: {
    backgroundColor: '#00E5FF', paddingVertical: 17, borderRadius: 999, alignItems: 'center',
    shadowColor: '#00E5FF', shadowOpacity: 0.45, shadowRadius: 18, shadowOffset: { width: 0, height: 5 },
    elevation: 8,
  },
  primaryBtnText: { color: '#000000', letterSpacing: 3, fontWeight: '800', fontSize: 12 },
  secondaryBtn: {
    borderWidth: 1, borderColor: 'rgba(255,255,255,0.18)', paddingVertical: 15, borderRadius: 999, alignItems: 'center', backgroundColor: '#15171D',
  },
  secondaryBtnText: { color: '#F3F4F6', fontWeight: '700', letterSpacing: 2.5, fontSize: 12 },
});
